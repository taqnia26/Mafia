...
router.post("/blackmarket", requireAuth, requireAlive, requireNotInPrison, async (req, res) => {
  try {
...
router.post("/blackmarket/:listingId/buy", requireAuth, requireAlive, requireNotInPrison, async (req, res) => {
  try {
    const clerkId = getCurrentClerkId(req);
    const player = await getOrCreatePlayer(clerkId);
...
router.delete("/blackmarket/:listingId", requireAuth, requireAlive, requireNotInPrison, async (req, res) => {
  try {
    const clerkId = getCurrentClerkId(req);
    const player = await getOrCreatePlayer(clerkId);
    const listingId = parseInt(String(req.params.listingId));
